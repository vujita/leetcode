...
const clap_node_ts_1 = require("./clap-node.cjs");
 ...
 */
class CommandNode extends clap_node_ts_1.ClapNode {
    ...
    getExecCommands(cmds, includeSubCommands) {
        ...
        if (this.cmdBase.exec && !(0, base_ts_1.isRootCommand)(this.alias)) {
            cmds.push(this);
        }
     ...
     */
    addCommandNode(node) {
        node[symbols_ts_1._PARENT] = this;
    ...
    applyDefaults() {
        ...
        for (const optName in options) {
            ...
            if (opt.spec.hasOwnProperty("argDefault") && !this.optNodes[optName]) {
                new node_generator_ts_1.ClapNodeGenerator(this).addOptionWithArgs(optName, [].concat(opt.spec.argDefault), opt);
            }
        }
        ...
        for (const key in config) {
            ...
            if (matchOpt) {
                ...
                if (!optNode || !optNode.source.startsWith("cli")) {
                    ...
                    new node_generator_ts_1.ClapNodeGenerator(this).addOptionWithArgs(matchOpt.name, [].concat(data.arg), matchOpt.option, src);
                }
            }
            else if (!this.optNodes[key]) {
                new node_generator_ts_1.ClapNodeGenerator(this).addOptionWithArgs(key, [].concat(data.arg), undefined, src);
            }
        }
    }
    /**
    ...
    makeCamelCaseOptions() {
        for (const key in this.optNodes) {
            const camelCaseKey = (0, xtil_ts_1.camelCase)(key);
...
//# sourceMappingURL=command-node.cjs.map