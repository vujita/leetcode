...
exports.ClapNodeGenerator = void 0;
const assert_1 = __importDefault(require("assert"));
const base_ts_1 = require("./base.cjs");
const command_node_ts_1 = require("./command-node.cjs");
const option_node_ts_1 = require("./option-node.cjs");
const xtil_ts_1 = require("./xtil.cjs");
const command_base_ts_1 = require("./command-base.cjs");
...
class ClapNodeGenerator {
    ...
    get cmdNode() {
        return this.node instanceof command_node_ts_1.CommandNode ? this.node : null;
    }
    get optNode() {
        return this.node instanceof option_node_ts_1.OptionNode ? this.node : null;
    }
    /**
     *
     * @param type
    ...
    convertValue(type, value, base) {
        ...
        if (type === "number") {
            ...
            return (0, xtil_ts_1.toBoolean)(value);
        }
        else {
            const customType = (_a = (base.spec.customTypes ||
                // @ts-expect-error - coercions is deprecated legacy property name, kept for backward compatibility
                base.spec.coercions)) === null || _a === void 0 ? void 0 : _a[type];
            ...
            if (typeof customType === "string") {
                ...
                this.node.addError(new Error(`argument '${value}' didn't match RegExp requirement for ${base.name}`));
            }
            else {
     ...
     * @returns
     */
    consumeNonOptAsCommand(arg, parsingCmd = "") {
        const cmdNode = this.cmdNode;
        const cmd = cmdNode.cmdBase;
        const createNewCommand = (matched) => {
            const node = new command_node_ts_1.CommandNode(matched.name, matched.alias, matched.cmd);
        ...
        if (matched.cmd) {
        ...
        else if ((ncConfig === null || ncConfig === void 0 ? void 0 : ncConfig.allowUnknownCommand) && (0, base_ts_1.isRootCommand)(this.cmdNode.alias)) {
            ...
            return createNewCommand({
                name: arg,
                alias: arg,
                cmd: ncConfig.allowUnknownOption ? command_base_ts_1.unknownCommandBase : command_base_ts_1.unknownCommandBaseNoOptions
            });
        }
        else {
            ...
            const requiresSubCommand = cmd.subCmdCount > 0 && !cmd.spec.exec && cmd.expectArgs === 0 && !hasMatchedSubCommand;
            ...
            if ((0, base_ts_1.isRootCommand)(this.cmdNode.alias) &&
                ...
                (ncConfig === null || ncConfig === void 0 ? void 0 : ncConfig.unknownCommandFallback) &&
                ...
                if (fallbackMatched.cmd) {
                    ...
                    const fallbackNode = new command_node_ts_1.CommandNode(fallbackMatched.name, fallbackMatched.alias, fallbackMatched.cmd);
            ...
            if (requiresSubCommand) {
                ...
                if (ncConfig === null || ncConfig === void 0 ? void 0 : ncConfig.allowUnknownCommand) {
                ...
                throw new base_ts_1.UnknownCliArgError(`Command '${cmd.name}' requires a subcommand. Unknown: '${arg}'.`, arg);
            }
            ...
            throw new base_ts_1.UnknownCliArgError(`Encountered unknown CLI argument '${arg}' while parsing for command '${parsingCmd}'.`, arg);
        }
        return [];
    }
    /**
    ...
    checkAutoBooleanOption(arg) {
        ...
        if (opt.expectArgs > 0 &&
            ...
            !(0, xtil_ts_1.isBoolean)(arg) // but incoming arg doesn't look like a boolean
        ) {
    ...
    consumeNonOptAsOption(arg) {
        ...
        (_a = this.parent.cmdNode) === null || _a === void 0 ? void 0 : _a.addVerbatimArg(arg);
     ...
     * @returns
     */
    setOptValue(data, complete = false, source = "cli") {
        var _a;
        ...
        let node;
        if (!matched) {
            const allowUnknownOption = (_a = this.cmdNode) === null || _a === void 0 ? void 0 : _a.cmdBase.allowUnknownOption;
            if (!allowUnknownOption) {
                ...
                this.node.addError(new base_ts_1.UnknownOptionError(`Encountered unknown CLI option '${data.name}'.`, data));
            }
            // no more parent, accept as unknown option at root command
            node = new option_node_ts_1.OptionNode(data, this.node);
        }
        else {
            node = new option_node_ts_1.OptionNode(matched, this.node);
        }
        node.source = source;
     ...
     * @param args
     */
    addOptionWithArgs(name, args, option, source = "default") {
    ...
    makeOptNode(data) {
        ...
        const optParentCmd = optNode.getParent();
    ...
    optEndGatherArgs() {
        ...
        const args = opt.args;
        (0, assert_1.default)(this.node.argsList && this.node.argsList.length >= opt.needArgs, `Not enough arguments for option '${opt.name}'`);
        const setArg = (argIx, arg, value) => {
        ...
        if (opt.hasArgs) {
            ...
            node.argsMap[0] =
                node.argsList.length > 0
                    ? this.convertValue((0, xtil_ts_1.isBoolean)(node.argsList[0]) ? "boolean" : (0, xtil_ts_1.isNumber)(node.argsList[0]) ? "number" : "string", node.argsList[0], opt)
                    : true;
        }
    ...
    cmdEndGatherArgs() {
        ...
        const args = cmd.args;
        (0, assert_1.default)(this.node.argsList && this.node.argsList.length >= cmd.needArgs, `Not enough arguments for command '${cmd.name}'`);
        const setArg = (argIx, arg, value) => {
            const setValue = Array.isArray(value)
...
exports.ClapNodeGenerator = ClapNodeGenerator;
//# sourceMappingURL=node-generator.cjs.map